<template>
  <div>
    ...
    <main :class="['home', { 'viewing-ticket': serviceTicketData }]">
      ...
      <div v-if="serviceTicketData" class="service-ticket">
        <div class="service-link">
          <a
            :href="
              `https://bloomington.in.gov/crm/tickets/view?ticket_id=${serviceTicketData[0].service_request_id}`
            "
            class="external"
            target="_blank"
            title="View ticket via uReport CRM"
            >View full ticket</a
          >
        </div>
        ...
        <template v-if="serviceTicketData[0].lat != null">
          <GmapMap
            ...
            :center="{
              lat: parseFloat(serviceTicketData[0].lat),
              lng: parseFloat(serviceTicketData[0].long)
            }"
            :zoom="19"
            map-type-id="hybrid"
            :options="{
        ...
        <div class="service-info">
          ...
          <div class="meta-row">
            <fn1-badge
              :variant="[
                {
                  success: serviceTicketData[0].status == 'open',
                  danger: serviceTicketData[0].status == 'closed'
                }
              ]"
            >
            ...
            <span
              v-if="serviceTicketData[0].agency_responsible != ''"
              class="meta has-icon"
            >
              <svg
                aria-hidden="true"
        ...
        <div class="service-body">
          <div
            class="description"
            v-if="serviceTicketData[0].description != null"
          >
      ...
      <emerModal />
      <p class="emergency"><strong>Note:</strong> If an Emergency, dial 911.</p>
    </main>
  </div>
</template>
...
export default {
  beforeRouteEnter(to, from, next) {
    next(vm => {
      ...
      if (!vm.initGroupData)
        vm.$axios
          .get(`${process.env.CRM_API_URL}${process.env.SERVICES_API}`)
          .then(res => {
      ...
      if (this.serviceTicketCRMHTML) {
        ...
        $(history).each(function(i, value) {
          let headingTxt = $(value)
            ...
            messageTxt = testMessagesTxt
              ? $(value)
                  .find(".historyItem .message")
                  .text()
              : null;
